...
Если PHP-сниппет обращается к `Config`, `PermissionHelper`, `CategoryPermissionService`, `CsrfProtection` или другим классам компонента **вне гарантированного bootstrap-контекста**, он обязан явно подключать:

```php
require_once MODX_CORE_PATH . 'components/testsystem/bootstrap.php';
```

Нельзя полагаться на внешнюю “магическую” автозагрузку. Принцип простой: если сниппет зависит от классов компонента, bootstrap должен быть подключён явно, если это не гарантировано вызывающим кодом.
...
Если сниппет может вызываться вне page context, сначала проверять, что `$modx->resource` существует, и только потом использовать `get(...)`. Fallback должен быть явным и безопасным для конкретного сценария: `''`, `site_start` или controlled error block.
...
**Правило:** в коде компонента использовать DB-API MODX напрямую: `$modx->prepare()`, `$modx->query()`, `$modx->exec()`, `$modx->lastInsertId()`.
...
### 3.5.1. Пользовательские и админские reporting-snippets не должны отдавать 500 при проблемах БД
...
- возвращать безопасный error block, пустое состояние или диагностическое сообщение, подходящее для UI.

Исключение: внутренние dev-only или staging-only diagnostic snippets, где допустим fail-fast для быстрой отладки.
...
Для изменений в header/menu/login/logout и role-based видимости обязательна ручная проверка минимум для ролей `anonymous`, `student` и `admin`:
...
Если в затронутом модуле есть дополнительные специальные роли, проверять и их тоже.
...
Если один и тот же визуальный блок используется в нескольких сниппетах (`addTestForm`, `csvImportForm`), его нельзя дублировать. HTML-разметка должна быть вынесена в общий chunk/partial или общий helper — в зависимости от архитектуры конкретного участка.
