...
В некоторых установках MODX 2.x метод `$modx->getPDO()` недоступен. Использование этого метода в сервисах приводит к фатальной ошибке и HTTP 500:

```
Call to undefined method modX::getPDO()
```

**Правило:** в коде компонента использовать DB-API MODX напрямую: `$modx->prepare()`, `$modx->query()`, `$modx->lastInsertId()`.
...
> ⚠️ Требуется проверка аудитором: подтвердить правило совместимости для всех целевых окружений (legacy MODX 2.x + текущий production).
...
Если в течение 14 дней появляется **2+ фикса одного и того же пользовательского симптома** (например, повторные исправления login/reset/step access), следующий PR по теме обязан включать:
1. краткий root-cause (1 абзац: почему баг возвращался),
2. минимальный regression-checklist в описании PR,
3. ссылку на конкретный слой, где зафиксирован инвариант (service/helper/template contract).

Без этих трёх пунктов фикс считается временным и не должен мержиться в `main`.

> ⚠️ Требуется проверка аудитором: подтвердить порог «2+ повтора за 14 дней» и формат обязательного regression-checklist для команды.

### 11.2. Контракт совместимости схемы перед релизом

Для модулей, где уже встречались расхождения схемы (`published` vs `publication_status`, `resource_id` и т.п.), перед merge обязателен lightweight-check:
...
> ⚠️ Требуется проверка аудитором: согласовать финальный список таблиц/полей, для которых schema-compat check обязателен всегда.
...
- таймауты и поведение при недоступном SMTP (fail-fast, без «зависших» HTTP-запросов);
- отсутствие побочных разрывов DB-соединения после отправки писем;
- понятное пользовательское сообщение при деградации внешнего сервиса.

Если вводится async-очередь писем, нужен documented fallback (что происходит при падении worker).

> ⚠️ Требуется проверка аудитором: утвердить единый SLA/таймауты для SMTP и критерии «безопасной деградации» auth-флоу.

### 11.4. UI/Template-кэш и роли: обязательный smoke-набор

Для изменений в header/menu/login/logout и role-based видимости обязательна ручная проверка минимум для ролей `student` и `admin`:
...
> ⚠️ Требуется проверка аудитором: подтвердить обязательный набор ролей и финальный smoke-сценарий для релизов UI-навигации.
...
> ⚠️ Требуется проверка аудитором: подтвердить, что единый поток импорта покрывает все legacy-сценарии без регрессий в UX и правах доступа.
