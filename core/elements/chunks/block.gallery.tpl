{* Чанк: block.gallery - Галерея с ресурса 14 для главной страницы
   Вызывается из: content_main.tpl
   Использует TV: elements из ресурса 14
*}
{var $gallery = 14 | resource : 'elements' | fromJSON}
<div class="row">
  {foreach $gallery as $idx => $galItem}
    {if $idx == 3}
    </div>
    <div class="hidden">
    {/if}
    <div class="col-xs-6 col-md-4">
      <br>
      <a href="{$galItem.img}" title="{$galItem.title | escape}" class="fancybox2"
        data-fancybox="gallery-{$_modx->resource.id}" rel="gallery-{$_modx->resource.id}">
        <img src="{$galItem.img | phpthumbon : "w=220&h=150&zc=1"}" alt="{$galItem.title | escape}" class="img-thumbnail">
      </a>
      <br>
    </div>
  {/foreach}
</div>