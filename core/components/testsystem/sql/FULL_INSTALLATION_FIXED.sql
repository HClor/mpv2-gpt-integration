...
-- ИСПРАВЛЕННАЯ ВЕРСИЯ ДЛЯ MySQL 5.7.21
-- ============================================
--
-- Этот файл был автоматически исправлен для совместимости с MySQL 5.7.21
-- Дата исправления: 2025-11-17 06:25:35
--
-- ОСНОВНЫЕ ИСПРАВЛЕНИЯ:
-- 1. Убрано IF NOT EXISTS из ALTER TABLE ADD COLUMN/INDEX
-- 2. Убрано IF NOT EXISTS из CREATE INDEX/TRIGGER/PROCEDURE
-- 3. Исправлены типы данных для Foreign Keys (INT(11) вместо INT(11) UNSIGNED)
-- 4. Исправлены названия полей (test_name → title, current_level → level)
-- 5. Убран WHERE clause из CREATE INDEX (не поддерживается в MySQL 5.7)
-- 6. CREATE OR REPLACE VIEW → DROP VIEW IF EXISTS + CREATE VIEW
-- 7. Исправлены ссылки на несуществующие поля (created_at → started_at, completed_at → finished_at)
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Категории тестов';
...
CREATE TABLE IF NOT EXISTS `modx_test_user_answers` (
    ...
    `question_id` INT(11) NOT NULL COMMENT 'ID вопроса',
    `answer_id` INT(11) DEFAULT NULL COMMENT 'ID выбранного ответа (для single)',
...
ALTER TABLE `modx_test_categories`
    ADD COLUMN `parent_id` INT(11) UNSIGNED DEFAULT NULL COMMENT 'Родительская категория' AFTER `id`,
...
CREATE TRIGGER `trg_category_permission_grant`
...
CREATE TRIGGER `trg_category_permission_modify`
...
CREATE TRIGGER `trg_category_permission_revoke`
...
CREATE TRIGGER `trg_enrollment_create_progress`
...
CREATE TRIGGER `trg_step_completion_update_progress`
...
ALTER TABLE `modx_test_user_answers`
    ADD COLUMN `answer_data` JSON DEFAULT NULL COMMENT 'Структурированные данные ответа (для новых типов вопросов)';
...
CREATE TRIGGER `trg_session_complete_award_xp`
...
CREATE TRIGGER `trg_xp_update_level`
...
CREATE PROCEDURE update_user_streak(IN p_user_id INT)
BEGIN
    ...
    ELSE
        ...
        IF v_last_date = v_today THEN
            ...
            -- Пропускаем обновление (RETURN не доступен в MySQL 5.7 для процедур)
            SET v_current_streak = v_current_streak; -- No-op
...
BEGIN
    ...
    IF NEW.level > OLD.level THEN
        ...
        VALUES (
            ...
            CONCAT('Вы достигли уровня ', NEW.level, ' - "', v_level_title, '"!'),
            'fa-level-up',
            'high',
            'level',
            NEW.level,
...
CREATE VIEW `modx_test_user_statistics` AS
SELECT
    u.id as user_id,
    u.username,
    ...
    COALESCE(ue.level, 1) as current_level,
...
CREATE VIEW `modx_test_test_statistics` AS
SELECT
    t.id as test_id,
    t.title,
    t.category_id,
    ...
    MAX(s.finished_at) as last_attempt_date
...
CREATE VIEW `modx_test_question_statistics` AS
SELECT
    q.id as question_id,
    q.test_id,
    q.question_text,
...
CREATE VIEW `modx_test_category_statistics` AS
SELECT
    c.id as category_id,
    c.name as category_name,
    c.parent_id,
...
BEGIN
    ...
    SELECT JSON_OBJECT(
        ...
        'last_activity', MAX(s.started_at)
    ) INTO v_metrics
    FROM modx_test_sessions s
    WHERE s.user_id = p_user_id
    AND (v_period_start IS NULL OR DATE(s.started_at) BETWEEN v_period_start AND v_period_end);
...
BEGIN
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN s.started_at IS NOT NULL THEN u.id END) as users_active,
        COUNT(DISTINCT s.id) as total_tests_taken,
        AVG(CASE WHEN s.status = 'completed' THEN s.score END) as avg_score,
        ROUND(COUNT(DISTINCT CASE WHEN s.started_at IS NOT NULL THEN u.id END) * 100.0 / COUNT(DISTINCT u.id), 2) as activation_rate
    FROM modx_users u
    LEFT JOIN modx_test_sessions s ON s.user_id = u.id
        AND DATE(s.started_at) BETWEEN p_start_date AND p_end_date
...
CREATE INDEX `idx_answers_analytics`
...
-- Индексы для оптимизации
CREATE INDEX `idx_cert_user_entity`
ON `modx_test_certificates` (`user_id`, `entity_type`, `entity_id`);

CREATE INDEX `idx_cert_issued_valid`
