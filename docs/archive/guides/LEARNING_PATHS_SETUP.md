...
5. ✅ Сниппет `learningPaths` создан
6. ⏳ Создай страницы MODX для UI
7. ⏳ Протестируй полный цикл обучения

## 📄 Создание страниц MODX

В админке MODX создайте следующие ресурсы:

### Страница 1: Список траекторий
```
Название: Траектории обучения
Alias: learning-paths
Template: (ваш основной шаблон с tsHead/tsScripts)
Content:
[[learningPaths]]
```

### Страница 2: Мои траектории
```
Название: Мои траектории
Alias: my-paths
Content:
[[learningPaths? &mode=`my`]]
```

### Страница 3: Просмотр траектории
```
Название: Траектория
Alias: learning-path
Content:
[[learningPaths? &mode=`view`]]
```
ID траектории передаётся через URL: `?id=X`

### Страница 4: Редактирование траектории
```
Название: Редактирование траектории
Alias: edit-path
Content:
[[learningPaths? &mode=`edit`]]
```
Доступна только для администраторов и ContentManager.

### Важно!
Шаблон страниц должен подключать чанки `tsHead` и `tsScripts` для работы JS:
```html
[[$tsHead]]
[[*content]]
[[$tsScripts]]
...
1. Перейти на `/learning-paths` - должен загрузиться список
2. Создать траекторию через UI
3. Добавить шаги (материалы, тесты)
4. Записать пользователя через `/edit-path?id=X`
5. Пройти траекторию как студент
6. Проверить выдачу достижений и сертификатов
