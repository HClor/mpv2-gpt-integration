# Test & Learning System v2.0 - Быстрая установка в MODX

Пошаговая инструкция по установке компонента Test & Learning System (система тестирования и онлайн-обучения) в MODX Revolution.
...
1. Создать дочерний ресурс для "tests":
   ...
   ```
   - **Опубликовано:** Да
   - **Описание:** Отображает список категорий слева и тесты выбранной категории справа
...
### 5.7. Создать страницу учебных материалов

1. Создать дочерний ресурс для "tests":
   - **Заголовок:** Учебные материалы
   - **Псевдоним:** learning
   - **Содержимое:**
   ```
   [[!learningMaterials]]
   ```
   - **Опубликовано:** Да
   - **Описание:** Отображает учебные материалы и онлайн-обучение с вопросами
...
Должна отобразиться страница с двумя колонками:
- **Слева:** Список категорий с количеством тестов
- **Справа:** Сообщение "Выберите категорию"

После клика на категорию справа должны отобразиться тесты этой категории.
...
### Проблема: Ошибка 500 "Could not find snippet getNotifications"

**Причина:** Сниппет `getNotifications` используется в chunk `tsHeader.tpl` для отображения уведомлений, но не был создан автоматически.

**Решение:**
1. Убедитесь, что файл `core/elements/snippets/getNotifications.php` существует
2. Запустите `install_elements.php` заново для регистрации сниппета в MODX
3. Или вручную создайте сниппет в MODX Manager:
   - **Название:** getNotifications
   - **Категория:** Test System
   - **Код:** скопируйте содержимое из `core/elements/snippets/getNotifications.php`
4. Очистите кеш MODX

**Альтернатива:** Если уведомления не нужны, закомментируйте строки 107-119 в `core/elements/chunks/tsHeader.tpl`
...
- [ ] База данных установлена (51 таблица, 14 триггеров, 12 процедур)
- [ ] Компонент зарегистрирован в MODX (namespace, settings, category)
- [ ] Snippets (33 шт.) и chunks (21 шт.) загружены из файлов
- [ ] Кеш MODX очищен
- [ ] Тестовые страницы созданы (включая `/tests/learning` для онлайн-обучения)
- [ ] CSS и JS подключены в шаблон
- [ ] Тестовые данные созданы
- [ ] Сниппет getNotifications создан и зарегистрирован
- [ ] Система протестирована и работает

---

**Поздравляем! Система тестирования и онлайн-обучения установлена! 🎉**
